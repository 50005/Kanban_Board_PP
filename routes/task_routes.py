...
from schemas.task import TaskCreate, TaskUpdate, Task  # Убедимся, что импортируем TaskCreate и TaskUpdate
...
def create_task():
    ...
    task = TaskCreate(**task_data)  # Передаем все данные из формы
    ...
    created_task = task_service.create_task(
        ...
        user_id=task.user_id,  # Передаем user_id
...
def update_task(task_id: int):
    task_data = request.form.to_dict()
    task = TaskUpdate(**task_data)  # Передаем все данные из формы
    db: Session = next(get_db())
    task_service = TaskService(db)
